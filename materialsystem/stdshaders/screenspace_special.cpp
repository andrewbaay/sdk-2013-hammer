...
#include "screenspaceefffect_vs20.inc"

// NOTE: This has to be the last file included!
#include "tier0/memdbgon.h"

BEGIN_SHADER_FLAGS( screenspace_special, "Help for screenspace_general", SHADER_NOT_EDITABLE )
	BEGIN_SHADER_PARAMS
	...
	SHADER_DRAW
	{
		...
		SHADOW_STATE
		{
			...
			{
				// Pre-cache shaders
				DECLARE_STATIC_VERTEX_SHADER( screenspaceefffect_vs20 );
				SET_STATIC_VERTEX_SHADER_COMBO( VERTEXCOLOR, bHasVertexColor );
				SET_STATIC_VERTEX_SHADER_COMBO( TRANSFORMVERTS, params[VERTEXTRANSFORM]->GetIntValue() );
				SET_STATIC_VERTEX_SHADER( screenspaceefffect_vs20 );
			}
...
//			if (params[ALPHATESTED]->GetFloatValue())
		...
		DYNAMIC_STATE
		{
			...
			if ( !bCustomVertexShader )
			{
				DECLARE_DYNAMIC_VERTEX_SHADER( screenspaceefffect_vs20 );
				SET_DYNAMIC_VERTEX_SHADER( screenspaceefffect_vs20 );
			}
		}
		Draw();
	}
