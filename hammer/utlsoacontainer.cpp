...
fltx4 NatExpSIMD(fltx4 x)
{
	fltx4 t, f, e, p, r;
	__m128i i, j;
	fltx4 l2e = _mm_set1_ps (1.442695041f);  /* log2(e) */
	fltx4 c0  = _mm_set1_ps (0.3371894346f);
	fltx4 c1  = _mm_set1_ps (0.657636276f);
	fltx4 c2  = _mm_set1_ps (1.00172476f);
		...
		for( int nY = m_nRowIndex; nY < m_pContainer->NumRows(); nY += QUANTIZER_NJOBS )
		{
			...
			do
			{
				...
				for( int n = 0; n < m_nNumResultsDesired; n++ )
				{
					...
					fltx4 fl4BetterMask = CmpLeSIMD( fl4TrialError, fl4ClosestError );
										 ...
										 int nChannelToReceiveErrorSignal )
{
	...
	while( nNumIterations-- )
	{
		...
		ParallelProcess( "DoKMeansWork", jobs, ARRAYSIZE( jobs ), DoKMeansWork );
									   ...
									   int nSrcField, int nDestField )
{
	...
	for( int x = nMinX; x <= nMaxX; x++ )
	{
		...
		flClosestDistance = MIN( flClosestDistance, (float)nSearchRadius );
