...
BEGIN_MESSAGE_MAP(CTreeList, CTreeCtrl)
	...
	ON_NOTIFY_REFLECT(TVN_BEGINDRAG, &ThisClass::OnBegindrag)
	ON_NOTIFY_REFLECT(TVN_ENDLABELEDIT, &ThisClass::OnEndLabelEdit)
	ON_NOTIFY_REFLECT(TVN_SELCHANGED, &ThisClass::OnSelChange)
	ON_NOTIFY_REFLECT(TVN_KEYDOWN, &ThisClass::OnKeyDown)
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_LBUTTONDBLCLK()
	ON_WM_RBUTTONDOWN()
