...
void CSoundBrowser::SaveValues()
{
	...
	AfxGetApp()->WriteProfileInt(s_pszSection, "Sound Volume", m_nVolume);
...
void CSoundBrowser::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_VOLUME, m_volumeControl);
	//}}AFX_DATA_MAP

	m_volumeControl.SetRange(0, 100, TRUE);
	m_volumeControl.SetPos(m_nVolume);
	g_Sounds.SetVolume(m_nVolume / 100.f);
}
...
BEGIN_MESSAGE_MAP(CSoundBrowser, CDialog)
	...
	ON_WM_CLOSE()
	ON_CBN_EDITCHANGE(IDC_SOUND_FILTER, &ThisClass::OnChangeFilter)
	ON_CBN_SELENDOK(IDC_SOUND_FILTER, &ThisClass::OnUpdateFilterNOW)
	ON_CBN_SELCHANGE(IDC_SOUND_TYPE, &ThisClass::OnSelchangeSoundType)
	ON_LBN_SELCHANGE(IDC_SOUND_LIST, &ThisClass::OnSelchangeSoundList)
	ON_LBN_DBLCLK(IDC_SOUND_LIST, &ThisClass::OnDblclkSoundList)
	ON_BN_CLICKED(IDC_PREVIEW, &ThisClass::OnPreview)
	ON_BN_CLICKED(IDC_AUTOPLAY, &ThisClass::OnAutoplay)
	ON_BN_CLICKED(IDC_STOPSOUND, &ThisClass::OnBnClickedStopsound)
	ON_BN_CLICKED(IDC_REFRESH_SOUNDS, &ThisClass::OnRefreshSounds)
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_OPEN_SOURCE, &ThisClass::OnOpenSource)
...
void CSoundBrowser::OnHScroll( UINT nSBCode, UINT nPos, CScrollBar* pScrollBar )
{
	if ( pScrollBar->m_hWnd == m_volumeControl.m_hWnd )
	{
		m_nVolume = m_volumeControl.GetPos();
		g_Sounds.SetVolume( m_nVolume / 100.f );
	}
}
