...
BOOL PrepareEmptyTempHammerDir( CString &sPath )
{
	...
	if ( !access( sPath, 00) )
	{
		...
		sprintf( bufPathCopy.Get(), "%s/*.*%c%c", (LPCSTR)sPath, 0, 0 );
...
{
	CAutoPushPop<bool> _auto_bSaveVisiblesOnly( m_pTempDoc->m_bSaveVisiblesOnly, false );
...
{
	if ( !((CMapEntity*)m_pEntity)->IsClass( "prop_static" ) )
		return ChunkFile_Ok;

	LPCTSTR szModelName = ((CMapEntity*)m_pEntity)->GetKeyValue( "model" );
