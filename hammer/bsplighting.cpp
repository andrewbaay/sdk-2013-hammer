	...
	CUtlVector<CFace> &faces )
{
	FOR_EACH_LL( m_FaceMaterials, i )
	{
		...
		FOR_EACH_LL( pMat->m_Faces, iFace )
		{
			...
			{
				CCoreDispInfo *pDisp = file.g_dispinfo[pFace->m_iDispInfo];

				int nTris  = pDisp->GetTriCount();
				int nVerts = pDisp->GetSize();
...
/*
	...
	for( int iFace=0; iFace < faces.Size(); iFace++ )
	{
		...
		CCoreDispInfo *pInDisp = file.g_dispinfo[pInFace->dispinfo];
		CDispInfoFaces *pOutDisp = &dispInfos[pInFace->dispinfo];

		pOutDisp->m_Power = pInDisp->GetPower();
		//int nVertsPerSide = (1 << pInDisp->power) + 1;

		pOutDisp->m_Verts.SetSize( pInDisp->GetSize() );

		int lightmapPageSize[2];
		pMatSys->GetLightmapPageSize( pFace->m_pStoredFace->m_LightmapPageID, &lightmapPageSize[0], &lightmapPageSize[1] );

		for( int iVert=0; iVert < pInDisp->GetSize(); iVert++ )
		{
			CVert *pOutVert = &pOutDisp->m_Verts[iVert];

			pOutVert->m_vPos = pInDisp->GetVert( iVert );
			Vector2D luxelCoord, texCoord;
			pInDisp->GetLuxelCoord( 0, iVert, luxelCoord );
			pInDisp->GetTexCoord( iVert, texCoord );
			for( int iCoord=0; iCoord < 2; iCoord++ )
			{
				float flVal = luxelCoord[iCoord];
				...
				pOutVert->m_vTexCoords[iCoord] = texCoord[iCoord];
