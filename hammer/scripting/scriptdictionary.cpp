...
constexpr asPWORD DICTIONARY_CACHE = 1003;
...
CScriptDictionary* CScriptDictionary::Create( asIScriptEngine* engine )
{
	...
	return new( obj ) CScriptDictionary( engine );
}
...
CScriptDictionary* CScriptDictionary::Create( asBYTE* buffer )
{
	// Use the custom memory routine from AngelScript to allow application to better control how much memory is used
	CScriptDictionary* obj = (CScriptDictionary*)asAllocMem( sizeof( CScriptDictionary ) );
	return new( obj ) CScriptDictionary( buffer );
}
...
CScriptDictionary::CScriptDictionary( asBYTE* buffer )
{
	...
	SDictionaryCache& cache = *reinterpret_cast<SDictionaryCache*>( engine->GetUserData( DICTIONARY_CACHE ) );
	bool keyAsRef = cache.keyType->GetFlags() & asOBJ_REF;
	...
	while ( length-- )
	{
		...
		if ( typeId >= asTYPEID_INT8 && typeId <= asTYPEID_DOUBLE )
		{
			...
			switch( typeId )
			{
			case asTYPEID_INT8:   i64 = *(int8*)		ref; break;
			case asTYPEID_INT16:  i64 = *(int16*)		ref; break;
			case asTYPEID_INT32:  i64 = *(int32*)		ref; break;
			case asTYPEID_INT64:  i64 = *(int64*)		ref; break;
			case asTYPEID_UINT8:  i64 = *(uint8*)		ref; break;
			case asTYPEID_UINT16: i64 = *(uint16*)		ref; break;
			case asTYPEID_UINT32: i64 = *(uint32*)		ref; break;
			case asTYPEID_UINT64: i64 = *(uint64*)		ref; break;
...
CScriptDictionary& CScriptDictionary::operator=( const CScriptDictionary& other )
{
	...
	for ( auto it = other.dict.cbegin(); it != other.dict.cend(); ++it )
	{
...
CScriptArrayT<ScriptString>* CScriptDictionary::GetKeys() const
{
	...
	CScriptArrayT<ScriptString>* array = static_cast<CScriptArrayT<ScriptString>*>( CScriptArray::Create( ti, asUINT( dict.size() ) ) );
	long current = -1;
	for ( auto it = dict.begin(); it != dict.end(); ++it )
	{
		current++;
		*array->At( current ) = it->first;
	}

	return array;
}

static void ScriptDictionaryFactory_Generic( asIScriptGeneric* gen )
{
	*(CScriptDictionary**)gen->GetAddressOfReturnLocation() = CScriptDictionary::Create( gen->GetEngine() );
}

static void ScriptDictionaryListFactory_Generic( asIScriptGeneric* gen )
{
...
void CScriptDictValue::Set( asIScriptEngine* engine, CScriptDictValue& value )
{
	if ( value.m_typeId & asTYPEID_OBJHANDLE )
		Set( engine, &value.m_valueObj, value.m_typeId );
	else if ( value.m_typeId & asTYPEID_MASK_OBJECT )
		Set( engine, value.m_valueObj, value.m_typeId );
	else
		Set( engine, &value.m_valueInt, value.m_typeId );
}
	...
	else
	{
		...
		else if ( typeId == asTYPEID_BOOL )
		{
			...
			else
			{
				...
				*(bool*)value = memcmp( &m_valueInt, &zero, size ) != 0;
			}
			return true;
		}
	}
...
CScriptDictionary::CIterator::CIterator( const CScriptDictionary& dict, dictMap_t::const_iterator it )
	: m_it(it), m_dict(dict)
{}
