...
struct CScriptGrid::SGridBuffer
{
	asDWORD width;
	asDWORD height;
	asBYTE  data[];
...
{
	// Allocate the memory
	void* mem = asAllocMem( sizeof( CScriptGrid ) );
	if ( !mem )
	{
	...
	return new( mem ) CScriptGrid( w, h, ti );
}
...
CScriptGrid* CScriptGrid::Create( asITypeInfo* ti, void* initList )
{
	...
	// Initialize the object
	return new( mem ) CScriptGrid( ti, initList );
}
...
CScriptGrid* CScriptGrid::Create( asITypeInfo* ti, asUINT w, asUINT h, void* defVal )
{
	...
	// Initialize the object
	return new( mem ) CScriptGrid( w, h, defVal, ti );
}
...
CScriptGrid::CScriptGrid( asITypeInfo* ti, void* buf )
{
	...
	if ( ( ti->GetSubTypeId() & asTYPEID_MASK_OBJECT ) == 0 )
	{
		...
		for ( asUINT y = 0; y < height; y++ )
		{
			...
			// Copy the line
			if ( width > 0 )
				V_memcpy( At( 0, y ), buf, width * elementSize );
	...
	else if ( ti->GetSubTypeId() & asTYPEID_OBJHANDLE )
	{
		...
		for ( asUINT y = 0; y < height; y++ )
		{
			...
			// its references too
			V_memset( buf, 0, width * elementSize );
...
bool CScriptGrid::CheckMaxSize( asUINT width, asUINT height )
{
	...
	asINT64 numElements = width * (asINT64)height;
...
void CScriptGrid::CreateBuffer( SGridBuffer** buf, asUINT w, asUINT h )
{
	...
	*buf = reinterpret_cast<SGridBuffer*>( asAllocMem( sizeof( SGridBuffer ) + elementSize * numElements ) );
...
void CScriptGrid::Construct( SGridBuffer* buf )
{
	...
	if ( subTypeId & asTYPEID_OBJHANDLE )
	{
		...
		V_memset( d, 0, ( buf->width * buf->height ) * sizeof( void* ) );
	}
...
void CScriptGrid::Release() const
{
	...
	if ( asAtomicDec( refCount ) == 0 )
	{
		...
		this->~CScriptGrid();
		asFreeMem( const_cast<CScriptGrid*>( this ) );
	}
}
