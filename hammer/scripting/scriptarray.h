...
protected:
	...
	bool	Equals( const void* a, const void* b, asIScriptContext* ctx, struct SArrayCache* cache ) const;
};

template <typename T>
class CScriptArrayT : public CScriptArray
{
public:
	T*			At( asUINT index ) { return static_cast<T*>( CScriptArray::At( index ) ); }
	const T*	At( asUINT index ) const { return static_cast<const T*>( CScriptArray::At( index ) ); }
};
