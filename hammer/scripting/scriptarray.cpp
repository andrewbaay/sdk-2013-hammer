...
struct CScriptArray::SArrayBuffer
{
	asDWORD maxElements;
	asDWORD numElements;
	asBYTE  data[];
...
CScriptArray* CScriptArray::Create( asITypeInfo* ti, asUINT length )
{
	...
	return new( mem ) CScriptArray( length, ti );
}
...
CScriptArray* CScriptArray::Create( asITypeInfo* ti, void* initList )
{
	...
	// Initialize the object
	return new( mem ) CScriptArray( ti, initList );
}
...
CScriptArray* CScriptArray::Create( asITypeInfo* ti, asUINT length, void* defVal )
{
	...
	// Initialize the object
	return new( mem ) CScriptArray( length, defVal, ti );
}
...
#if !AS_USE_STLNAMES && AS_USE_ACCESSORS
...
CScriptArray::CScriptArray( asITypeInfo* ti, void* buf )
{
	...
	Assert( ti && V_strcmp( ti->GetName(), "array" ) == 0 );
	...
	if ( ( ti->GetSubTypeId() & asTYPEID_MASK_OBJECT ) == 0 )
	{
		...
		if ( length > 0 )
			V_memcpy( At( 0 ), (asUINT*)buf + 1, length * elementSize );
	}
	else if ( ti->GetSubTypeId() & asTYPEID_OBJHANDLE )
	{
		...
		// its references too
		V_memset( (asUINT*)buf + 1, 0, length * elementSize );
	}
	else if ( ti->GetSubType()->GetFlags() & asOBJ_REF )
	{
		...
		// Copy the handles into the internal buffer
		if ( length > 0 )
			V_memcpy( buffer->data, (asUINT*)buf + 1, length * elementSize );
...
void CScriptArray::Reserve( asUINT maxElements )
{
	...
	SArrayBuffer* newBuffer = reinterpret_cast<SArrayBuffer*>( asAllocMem( sizeof( SArrayBuffer ) + elementSize * maxElements ) );
	if ( newBuffer )
	{
	...
	V_memcpy( newBuffer->data, buffer->data, buffer->numElements * elementSize );
...
void CScriptArray::RemoveRange( asUINT start, asUINT count )
{
	...
	// since we're just copying the pointers to objects and not the actual objects.
	V_memmove( buffer->data + start * elementSize, buffer->data + ( start + count ) * elementSize, ( buffer->numElements - start - count ) * elementSize );
...
void CScriptArray::Resize( int delta, asUINT at )
{
	...
	if ( buffer->maxElements < buffer->numElements + delta )
	{
		...
		// As objects in arrays of objects are not stored inline, it is safe to use memcpy here
		// since we're just copying the pointers to objects and not the actual objects.
		V_memcpy( newBuffer->data, buffer->data, at * elementSize );
		if ( at < buffer->numElements )
			V_memcpy( newBuffer->data + ( at + delta ) * elementSize, buffer->data + at * elementSize, ( buffer->numElements - at ) * elementSize );
	...
	else if ( delta < 0 )
	{
		...
		// As objects in arrays of objects are not stored inline, it is safe to use memmove here
		// since we're just copying the pointers to objects and not the actual objects.
		V_memmove( buffer->data + at * elementSize, buffer->data + ( at - delta ) * elementSize, ( buffer->numElements - ( at - delta ) ) * elementSize );
	...
	else
	{
		// As objects in arrays of objects are not stored inline, it is safe to use memmove here
		// since we're just copying the pointers to objects and not the actual objects.
		V_memmove( buffer->data + ( at + delta ) * elementSize, buffer->data + at * elementSize, ( buffer->numElements - at ) * elementSize );
...
void CScriptArray::Construct( SArrayBuffer* buf, asUINT start, asUINT end )
{
		...
		for ( ; d < max; d++ )
		{
			*d = engine->CreateScriptObject( subType );
			if ( !*d )
			{
				...
				V_memset( d, 0, sizeof( void* ) * ( max - d ) );
	...
	else
	{
		...
		V_memset( d, 0, ( end - start ) * elementSize );
	}
}
		...
		if ( !cache || ( cache->cmpFunc == nullptr && cache->eqFunc == nullptr ) )
		{
			...
			if ( ctx )
			{
				...
				if ( cache && cache->eqFuncReturnCode == asMULTIPLE_FUNCTIONS )
					V_sprintf_safe( tmp, "Type '%s' has multiple matching opEquals or opCmp methods", subType->GetName() );
				else
					V_sprintf_safe( tmp, "Type '%s' does not have a matching opEquals or opCmp method", subType->GetName() );
...
void CScriptArray::Sort( asUINT startAt, asUINT count, bool asc )
{
		...
		if ( !cache || cache->cmpFunc == nullptr )
		{
			...
			if ( ctx )
			{
				...
				if ( cache && cache->cmpFuncReturnCode == asMULTIPLE_FUNCTIONS )
					V_sprintf_safe( tmp, "Type '%s' has multiple matching opCmp methods", subType->GetName() );
				else
					V_sprintf_safe( tmp, "Type '%s' does not have a matching opCmp method", subType->GetName() );
	...
	if ( subTypeId & ~asTYPEID_MASK_SEQNBR )
	{
		...
		} customLess = { asc, cmpContext, cache ? cache->cmpFunc : nullptr };
	...
	else
	{
		if ( dst->numElements > 0 && src->numElements > 0 )
		{
			...
			else
			{
				...
				V_memcpy( dst->data, src->data, count * elementSize );
			}
		}
	}
}
...
void CScriptArray::Precache()
{
	...
	V_memset( cache, 0, sizeof( SArrayCache ) );

	// If the sub type is a handle to const, then the methods must be const too
	bool mustBeConst = ( subTypeId & asTYPEID_HANDLETOCONST ) != 0;
	...
	if ( subType )
	{
		for ( asUINT i = 0; i < subType->GetMethodCount(); i++ )
		{
			...
			if ( func->GetParamCount() == 1 && ( !mustBeConst || func->IsReadOnly() ) )
			{
				...
				if ( returnTypeId == asTYPEID_INT32 && V_strcmp( func->GetName(), "opCmp" ) == 0 )
					isCmp = true;
				if ( returnTypeId == asTYPEID_BOOL && V_strcmp( func->GetName(), "opEquals" ) == 0 )
					isEq = true;
