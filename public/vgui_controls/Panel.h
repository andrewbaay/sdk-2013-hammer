...
#define DECLARE_PANEL_SETTINGS()									\
	public:															\
	virtual size_t GetNumPanelSettings() const;						\
	virtual const vgui::PanelSetting_t& GetSetting(size_t setting) const;

#define BEGIN_PANEL_SETTINGS(classname)			\
	static constexpr const vgui::PanelSetting_t classname##Settings[] = {

#define END_PANEL_SETTINGS(classname)	\
	};	\
	size_t classname::GetNumPanelSettings() const { return BaseClass::GetNumPanelSettings() + ARRAYSIZE( classname##Settings ); }	\
	const vgui::PanelSetting_t& classname::GetSetting(size_t setting) const { return setting < BaseClass::GetNumPanelSettings() ? BaseClass::GetSetting( setting ) : classname##Settings[setting - BaseClass::GetNumPanelSettings()]; }
...
public:
	virtual size_t GetNumPanelSettings() const;						\
	virtual const PanelSetting_t& GetSetting( size_t setting ) const;
private:
