...
{
	FLTX4 value = _mm_set_ss( flValue );
	return _mm_shuffle_ps( value, value, 0 );
}
...
#if !defined(POSIX) && !defined(WIN32_CLANG)
...
{
	return static_cast<uint32>( SubFloat( a, idx ) );
}
...
FORCEINLINE int32 SubSInt( const fltx4& a, int idx )
{
#if !defined(POSIX) && !defined(WIN32_CLANG)
	return a.m128_i32[idx];
#else
	return reinterpret_cast<int32 const*>( &a )[idx];
#endif
}

FORCEINLINE int32& SubSInt( fltx4& a, int idx )
{
#if !defined(POSIX) && !defined(WIN32_CLANG)
	return a.m128_i32[idx];
#else
	return reinterpret_cast<int32*>( &a )[idx];
#endif
}

// Return one in the fastest way -- on the x360, faster even than loading.
FORCEINLINE fltx4 LoadZeroSIMD()
{
	return Four_Zeros;
}

// Return one in the fastest way -- on the x360, faster even than loading.
FORCEINLINE fltx4 LoadOneSIMD()
{
	return Four_Ones;
}
...
{
	return MaskedAssign( LoadAlignedSIMD( g_SIMD_ComponentMask[0] ), x, a );
}

FORCEINLINE fltx4 SetYSIMD( const fltx4& a, const fltx4& y )
{
	return MaskedAssign( LoadAlignedSIMD( g_SIMD_ComponentMask[1] ), y, a );
}

FORCEINLINE fltx4 SetZSIMD( const fltx4& a, const fltx4& z )
{
	return MaskedAssign( LoadAlignedSIMD( g_SIMD_ComponentMask[2] ), z, a );
}

FORCEINLINE fltx4 SetWSIMD( const fltx4& a, const fltx4& w )
{
	return MaskedAssign( LoadAlignedSIMD( g_SIMD_ComponentMask[3] ), w, a );
}

FORCEINLINE fltx4 SetComponentSIMD( const fltx4& a, int nComponent, float flValue )
{
	FLTX4 val = ReplicateX4( flValue );
	return MaskedAssign( LoadAlignedSIMD( g_SIMD_ComponentMask[nComponent] ), val, a );
}
...
{
	FLTX4 m = MulSIMD( a, b );
	const float flDot = SubFloat( m, 0 ) + SubFloat( m, 1 ) + SubFloat( m, 2 );
...
{
	FLTX4 m = MulSIMD( a, b );
	const float flDot = SubFloat( m, 0 ) + SubFloat( m, 1 ) + SubFloat( m, 2 ) + SubFloat( m, 3 );
	return ReplicateX4( flDot );
}
...
{
	fltx4 retval;
	SubFloat( retval, 0 ) = ( (float)SubSInt( vSrcA, 0 ) );
	SubFloat( retval, 1 ) = ( (float)SubSInt( vSrcA, 1 ) );
	SubFloat( retval, 2 ) = ( (float)SubSInt( vSrcA, 2 ) );
	SubFloat( retval, 3 ) = ( (float)SubSInt( vSrcA, 3 ) );
	return retval;
}
...
{
	const __m64 bottom = _mm_cvttps_pi32( vSrc );
	const __m64 top    = _mm_cvttps_pi32( _mm_movehl_ps(vSrc,vSrc) );
...
public:
	...
	{
		// NOTE: if the output goes into a register, this causes a Load-Hit-Store stall (don't mix fpu/vpu math!)
		return SubFloat( const_cast<fltx4&>( x ), idx );
	}

	FORCEINLINE const float& Y(int idx) const
	{
		return SubFloat( const_cast<fltx4&>( y ), idx );
	}

	FORCEINLINE const float& Z(int idx) const
	{
		return SubFloat( const_cast<fltx4&>( z ), idx );
	}
	...
	FORCEINLINE FourVectors& operator=( FourVectors const& src )
	{
		x=src.x;
		y=src.y;
		z=src.z;
...
FORCEINLINE fltx4 RandSignedSIMD()					// -1..1
{
...
{
	FLTX4 fl4Abs = fabs( val );
	fltx4 ival = SubSIMD( AndSIMD( LoadAlignedSIMD( g_SIMD_lsbmask ), AddSIMD( fl4Abs, Four_2ToThe23s ) ), Four_2ToThe23s );
