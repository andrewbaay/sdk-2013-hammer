	...
	for ( int i = 0; i < seqdesc.numautolayers; i++ )
	{
		...
		if ( pLayer->start != pLayer->end )
		{
			...
			else
			{
				...
				if ( iPose != -1 )
				{
					const mstudioposeparamdesc_t &Pose = const_cast<CStudioHdr *>( pStudioHdr )->pPoseParameter( iPose );
...
)
{
	...
	mstudioseqdesc_t	&seqdesc = const_cast<CStudioHdr *>( pStudioHdr )->pSeqdesc( sequence );
