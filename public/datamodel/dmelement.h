...
#define DEFINE_ELEMENT( className, baseClassName )	\
	public:											\
		...
		{											\
			...
			return IsA( typeSymbol );				\
													...
													\
		template< class T_ > bool IsA() const		\
		{											\
			return IsA( T_::GetStaticTypeSymbol() ); \
		}											\
													\
		virtual int GetInheritanceDepth( UtlSymId_t typeSymbol ) const	\
		{											\
	...
	private:																					\
		typedef baseClassName BaseClass; 														\
		template <class T_> friend class CDmElementFactory;										\
		template <class T_> friend class CDmAbstractElementFactory;										\
		static CUtlSymbol m_classType
