...
{
public:
	const char* GetName() const override { return "vmt"; }
	const char* GetDescription() const override { return "Valve Material File"; }
...
bool CImportVMT::SerializeShaderParameters( CUtlBuffer& buf, CDmElement* pRoot )
{
	for ( CDmAttribute* pAttribute = pRoot->FirstAttribute(); pAttribute; pAttribute = pAttribute->NextAttribute() )
	{
		...
		if ( !V_stricmp( pAttribute->GetName(), "shader" ) )
			continue;
...
//-----------------------------------------------------------------------------
static int ParseVectorFromKeyValueString( const char* pParamName, const char* pScan, const char* pMaterialName, Vector4D& vecVal )
{
...
{
	Vector4D vecVal;
	const int nDim = ParseVectorFromKeyValueString( pParamName, pString, FileName(), vecVal );
	if ( nDim == 0 )
		return false;
...
bool CImportVMT::CreateMatrixMaterialVarFromKeyValue( CDmElement* pElement, const char* pParamName, const char* pScan )
{
	...
	VMatrix mat;
	int count = sscanf_s( pScan, " [ %f %f %f %f  %f %f %f %f  %f %f %f %f  %f %f %f %f ]",
...
{
	char pParamName[512];
	V_strcpy_safe( pParamName, pKeyValues->GetName() );
	V_strlower( pParamName );
...
{
	int count = 0;
	while ( count < 10 && V_stricmp( pKeyValues->GetName(), "patch" ) == 0 )
	{
//		WriteKeyValuesToFile( "patch.txt", keyValues );
		...
		if ( pIncludeFileName )
		{
			...
			const bool success = includeKeyValues->LoadFromFile( g_pFullFileSystem, pIncludeFileName, nullptr );
			if ( success )
			{
...
CDmElement* CImportVMT::UnserializeFromKeyValues( KeyValues* pKeyValues )
{
	...
	{
		if ( !V_stricmp( pSubKey->GetName(), "Proxies" ) )
