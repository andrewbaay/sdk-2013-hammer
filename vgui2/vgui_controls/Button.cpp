...
void Button::Init()
{
	...
	m_sArmedSoundName = NULL;
	m_sDepressedSoundName = NULL;
	m_sReleasedSoundName = NULL;
	SetTextInset(6, 0);
...
void Button::ApplySettings( KeyValues *inResourceData )
{
	...
	bool bNotFound;
	const bool bSelected = inResourceData->GetBool( "selected", false, &bNotFound );
	if ( !bNotFound )
	{
