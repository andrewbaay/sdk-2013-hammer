...
BEGIN_PANEL_SETTINGS( TextEntry )
	{ "font", TYPE_STRING },
...
//	surface()->DrawFilledRect(0, 0, wide, tall);
	...
	bool composing = m_bAllowNonAsciiCharacters && m_szComposition[0] != L'\0';
...
void TextEntry::OnCreateDragData( KeyValues *msg )
{
	...
	{
		int len = r1 - r0;
		char* selection = (char*)stackalloc( ( len + 1 ) );
		GetTextRange(selection, r0, len);
...
int TextEntry::GetStartDrawIndex(int &lineBreakIndexIndex)
{
	...
	if (startLine > 0)
	{
		...
		if (startLine < m_LineBreaks.Count())
		{
...
void TextEntry::ApplySettings( KeyValues *inResourceData )
{
	...
	SetTextHidden(inResourceData->GetBool("textHidden"));
