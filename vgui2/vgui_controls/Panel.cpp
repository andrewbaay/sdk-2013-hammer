...
/*PanelSetting_t::PanelSetting_t()
{
	V_strncpy( name, "", sizeof( name ) );
	type = TYPE_STRING;
}

PanelSetting_t::PanelSetting_t( const char *pName, PanelSettingType_t t )
{
...
size_t Panel::GetNumPanelSettings() const { return ARRAYSIZE( PanelSettings ); }
const PanelSetting_t& Panel::GetSetting(size_t setting) const { return PanelSettings[setting]; };
...
bool Panel::IsOpaque()
{
	...
	return IsVisible() && _flags.IsFlagSet( PAINT_BACKGROUND_ENABLED ) && ( _bgColor[3] == 255 );
}
...
void CBoxSizer::DoLayout( int BaseX, int BaseY, int SizeX, int SizeY )
{
	...
	for( int i = 0; i < m_Members.Count(); ++i )
	{
		...
		{
			nExpansion = MAX( 0.f, (flMemberExpand/flExpandSum) * nMajorRemainder );
		}
...
void Panel::ApplySettings(KeyValues *inResourceData)
{
	...
	_buildModeFlags.SetFlag( BUILDMODE_SAVE_XPOS_RIGHTALIGNED
						...
						| BUILDMODE_SAVE_YPOS_PROPORTIONAL_SELF, false );
...
template< class S >
void Panel::PostMessageToAllSiblingsOfType( KeyValues *msg, float delaySeconds /*= 0.0f*/ )
{
