...
BEGIN_PANEL_SETTINGS( RichText )
	{ "text", TYPE_STRING },
	{ "textfile", TYPE_STRING },
	{ "maxchars", TYPE_INTEGER },
	{ "scrollbar", TYPE_BOOL }
END_PANEL_SETTINGS( RichText );
...
static FORCEINLINE int GetStringWidth( const CUtlVector<wchar_t> &textStream, HFont font, int start, int end )
{
	int charWide = 0;
...
//	VPROF( "RichText::DrawString" );
	...
	const bool gotSelRange = GetSelectedRange( selection0, selection1 );
	const int lineLength = iLast - iFirst + 1;

	const bool startsHighlight = ( iFirst < selection0 && iLast > selection0 );
	const bool isHighlight = ( iFirst >= selection0 && iLast <= selection1 );
	const bool endsHighlight = ( iFirst < selection1 && iLast > selection1 );
	...
	if ( gotSelRange && selection0 != selection1 && ( startsHighlight || isHighlight || endsHighlight ) )
	{
		...
		else
		{
			...
			if ( iLast < selection1 )
			{
				...
				const int normalTextLen = selection0 - iFirst;
				const int normalTextWidth = GetStringWidth( m_TextStream, font, iFirst, selection0 );
				surface()->DrawSetTextColor( renderState.textColor );
				if ( renderState.textColor.a() != 0 )
				{
					m_bAllTextAlphaIsZero = false;
				...
				const int highlightedTextLen = lineLength - normalTextLen;
				const int highlightedTextWidth = GetStringWidth( m_TextStream, font, selection0, iLast + 1 );
				surface()->DrawSetColor( _selectionColor );
			...
			else
			{
				...
				const int normalText1Length = selection0 - iFirst;
				const int normalText1Width = GetStringWidth( m_TextStream, font, iFirst, selection0 );
				surface()->DrawSetTextColor( renderState.textColor );
				...
				const int highlightedTextLen = selection1 - selection0;
				const int highlightedTextWidth = GetStringWidth( m_TextStream, font, selection0, selection1 );
				surface()->DrawSetColor( _selectionColor );
				...
				const int normalText2Length = iLast - selection1;
				surface()->DrawSetTextColor( renderState.textColor );
...
void RichText::CalculateFade( TRenderState &renderState )
{
	if ( m_FormatStream.IsValidIndex( renderState.formatStreamIndex ) )
	{
		...
		{
			TFade& fade = m_FormatStream[renderState.formatStreamIndex].fade;
