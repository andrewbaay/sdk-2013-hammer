...
void Label::ApplySchemeSettings(IScheme *pScheme)
{
	...
	if ( _fontOverrideName.IsEmpty() )
		_fontOverrideName = pScheme->GetFontName( GetFont() );
...
void Label::GetSettings( KeyValues *outResourceData )
{
	...
	if ( m_bUseProportionalInsets )
	{
		...
		outResourceData->SetInt("textinsety", scheme()->GetProportionalNormalizedValueEx( GetScheme(), _textInset[1] ) );
	}
	else
	{
...
void Label::ApplySettings( KeyValues *inResourceData )
{
	...
	_associateName = inResourceData->GetString("associate", NULL);

	if (inResourceData->GetBool("dulltext"))
	{
		SetTextColorState(CS_DULL);
	}
	else if (inResourceData->GetBool("brighttext"))
	{
	...
	bool bWrapText = inResourceData->GetBool("centerwrap");
	SetCenterWrap( bWrapText );

	m_bAutoWideToContents = inResourceData->GetBool("auto_wide_tocontents");
    m_bAutoTallToContents = inResourceData->GetBool("auto_tall_tocontents");

	bWrapText = inResourceData->GetBool("wrap");
	SetWrap( bWrapText );
	...
	m_bUseProportionalInsets = inResourceData->GetBool("use_proportional_insets");
	if ( m_bUseProportionalInsets )
	{
	...
	bool bAllCaps = inResourceData->GetBool("allcaps");
	SetAllCaps( bAllCaps );
