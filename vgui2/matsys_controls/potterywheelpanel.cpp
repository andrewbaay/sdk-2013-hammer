...
{
	if ( idx >= 0 && idx < m_nLightCount )
	{
		// Update the existing light
		SetIdentityMatrix( m_Lights[idx].m_LightToWorld );
		m_Lights[idx].m_Desc.m_Type = MATERIAL_LIGHT_DIRECTIONAL;
		m_Lights[idx].m_Desc.m_Color.Init( color.r() / 255.0f, color.g() / 255.0f, color.b() / 255.0f );
		m_Lights[idx].m_Desc.m_Direction.Init( direction.x, direction.y, direction.z );
		m_Lights[idx].m_Desc.m_Range=0.0;
		m_Lights[idx].m_Desc.m_Attenuation0 = 1.0;
		m_Lights[idx].m_Desc.m_Attenuation1 = 0;
		m_Lights[idx].m_Desc.m_Attenuation2 = 0;
		m_Lights[idx].m_Desc.RecalculateDerivedValues();
	}
	else
	{
...
void CPotteryWheelPanel::AddDirectionalLight( const Color& color, const Vector& direction )
{
	...
	{
		SetIdentityMatrix( m_Lights[m_nLightCount].m_LightToWorld );
		m_Lights[m_nLightCount].m_Desc.m_Type = MATERIAL_LIGHT_DIRECTIONAL;
		m_Lights[m_nLightCount].m_Desc.m_Color.Init( color.r() / 255.0f, color.g() / 255.0f, color.b() / 255.0f );
		m_Lights[m_nLightCount].m_Desc.m_Direction.Init( direction.x, direction.y, direction.z );
		m_Lights[m_nLightCount].m_Desc.m_Range=0.0;
		m_Lights[m_nLightCount].m_Desc.m_Attenuation0 = 1.0;
		m_Lights[m_nLightCount].m_Desc.m_Attenuation1 = 0;
		m_Lights[m_nLightCount].m_Desc.m_Attenuation2 = 0;
		m_Lights[m_nLightCount].m_Desc.RecalculateDerivedValues();
		m_nLightCount++;
	}
}
...
void CPotteryWheelPanel::SetLightAmbient( const Vector& ambient )
{
	for ( int i = 0; i < 6; ++i )
	{
		m_vecAmbientCube[i].Init( ambient.x, ambient.y, ambient.z );
	}
}
...
void CPotteryWheelPanel::RenderCapture()
{
	...
	Frustum frustum;
	CViewSetupEngine view2d;
	view2d.x				= 0;
	view2d.y				= 0;
	view2d.width			= m_nRenderWidth;
	view2d.height			= m_nRenderHeight;
	pCfg->m_pIVRenderView->Push3DView( view2d, VIEW_CLEAR_COLOR | VIEW_CLEAR_DEPTH, pCfg->m_pDummyColorBufferTexture, frustum );
	dynamic_cast<ITextureInternal*>( pCfg->m_pDummyColorBufferTexture )->SetRenderTarget( 0, pCfg->m_pFlashlightDepthTexture );
	...
	pCfg->m_pIVRenderView->PopView( frustum );
